...
def main(argv):
    ...
    no_of_elements=len(data_set[0]);  # no of elements in one of the file  (total/3)
   #sorted_data_ins_sort=insertion_sort(data_set);
	...
	# list that holds the jobs
        ...
        sort_india_wrapper = saga.filesystem.File('file://localhost/%s/scatter_india.sh' % os.getcwd())
        sort_india_wrapper.copy(workdir_1.get_url())
        ...
        sort_sierra_wrapper = saga.filesystem.File('file://localhost/%s/scatter_sierra.sh' % os.getcwd())
        sort_sierra_wrapper.copy(workdir_2.get_url())
        sort_sierra_exe = saga.filesystem.File('file://localhost/%s/insertion_sort.py' % os.getcwd())
	...
	# describe our job
       ...
       # jd.environment     = {'MYOUTPUT':'"Hello from SAGA"'}
        ...
        jd_1.executable      = 'sh'
        jd_1.arguments       = ['scatter_india.sh']
...
# describe our job
       ...
       # jd.environment     = {'MYOUTPUT':'"Hello from SAGA"'}
        ...
        jd_2.executable      = 'sh'
        jd_2.arguments       = ['scatter_sierra.sh']
...
# describe our job
        jd_3 = saga.job.Description()
        outputfile='Sorted_Data3.txt'	
        # Next, we describe the job we want to run. A complete set of job
        # description attributes can be found in the API documentation.
        
       # jd.environment     = {'MYOUTPUT':'"Hello from SAGA"'}
        jd_3.total_cpu_count =1
        jd_3.wall_time_limit = 10
        jd_3.working_directory = workdir_3.get_url().path
        jd_3.executable      = 'sh'
        jd_3.arguments       = ['scatter_alamo.sh']
        jd_3.output          = "mysagajob.stdout"
        jd_3.error           = "mysagajob.stderr"

        # Create a new job from the job description. The initial state of
        # the job is 'New'.
	job_3 = jobservice_3.create_job(jd_3)	
	
		
	
	
        # Check our job's id and state
        print "Job ID    : %s" % (job_1.id)
        print "Job State : %s" % (job_1.state)

        print "\n...starting job...\n"

        print "Job ID    : %s" % (job_2.id)
        ...
        print "Job State : %s" % (job_2.state)        
        job_3.run()
        print "Job ID    : %s" % (job_3.id)
        print "Job State : %s" % (job_3.state)        
        ...
        print "Job State : %s" % (job_3.state)
        print "Exitcode  : %s" % (job_3.exit_code)
        ...
        outfilesource3 = 'sftp://dino24@alamo.futuregrid.org/N/u/dino24/sort_related/Sorted_Data3.txt' #sftp://
        outfiletarget3 = 'file://localhost/tmp/'
        out3 = saga.filesystem.File(outfilesource3, session=session3)
        out3.copy(outfiletarget3)
         
        print "Staged out %s to %s (size: %s bytes)" % (outfilesource3, outfiletarget3, out3.get_size())
